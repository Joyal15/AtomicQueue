...
**Status (audited against the repo, 2026-09-03): 🟡 In progress, not exit-criteria-clean yet.** Both tracks have real, correct code behind several items — this isn't stub work — but one cross-cutting gap blocks the exit criteria outright: **the `authenticate` session middleware exists and is correct, but is exported from nowhere and wired into zero routes**, on either track. Until it's on `auth/index.ts`'s barrel and mounted, every `req.user.businessId` read in `services`/`resources`/`availability`/`providers` throws at runtime (TS doesn't catch it — `Express.Request.user` is typed optional and `strictNullChecks` isn't on), and there's no way to reach a protected route at all since there's also no login endpoint yet.
...
- [~] RBAC middleware + businessId-scoping middleware, exported for Peter 3 — *`authenticate.ts` covers businessId-scoping correctly (populates `req.user.businessId` from a fresh Mongo read, never trusts the session record for it).* **Missing two things: (1) no owner-vs-staff RBAC check middleware exists at all (not yet needed by any built route, but it's on the checklist); (2) `authenticate` is not exported from `auth/index.ts` — the module's barrel only exports `authRouter` — and is consequently imported/used nowhere in the codebase, including auth's own routes.** This is the root cause of Peter 3's biggest Phase 2 gap below — Peter 3 has nothing importable to protect routes with yet, and (a side effect worth naming) `providers.service.ts` currently reaches into `auth.model.ts`'s `UserModel` directly rather than through `auth`'s exports, because `auth` doesn't expose a usable read function either — swap that once `auth` has something to import.
...
- [~] `Business` schema + `tenants` module: transactional-insert export + unique slug routing — *verified in repo: `tenants.service.ts`'s `createBusiness(input, session)` matches the shared-touchpoint-5 contract precisely (throws if not called inside an open transaction, uses array-form `create` so it enrolls in the caller's session); `Businesses.slug` has `unique: true`.* **Missing: `BusinessDocument` (and the shared `Business` type) has only `name`/`slug`/`ownerId` — no `timezone` or `cancellationCutoffMinutes`, both required by architecture §2 (every `Slot.datetime` conversion in Phase 3 depends on `timezone` existing).** `tenants.routes.ts`/`tenants.controller.ts` are still the original skeleton (`GET /status` only) — no real business-management endpoint yet.
- [x] `Service` schema + `services` module: CRUD, businessId-scoped — *verified in repo: full create/list/get/update/deactivate, every query filtered by `businessId`.* (Not yet reachable without the `authenticate` gap above closed.)
- [x] `Resources` schema + CRUD — *verified in repo: full CRUD, `status: 'active'|'removed'`, `capacity`, correct.* **Location deviates from the plan's own instruction** — this lives in its own `resources/` module rather than the `providers` module the plan names ("don't split it ambiguously across `staff`/`resources`"). Works correctly; flagging as a deliberate-or-not decision to make, not a defect.
...
- [x] `ProviderAvailability` schema + CRUD + weekly template — *verified in repo: schema + full CRUD in `availability/`, `serviceId` validated same-business+active, `providerId`/`providerType` validated via the function above.* **Same location deviation as Resources** — lives in `availability/`, not `providers/` as the plan names it.
- [ ] `StaffInvitations` schema + invitation send/accept flow — **not built at all**, nothing in the repo. The plan calls this "not optional CRUD, it's the only path to a staff account" — so the phase's own exit criteria ("adds... a staff member") cannot be met until this exists.
- [~] Publish `Business`/`Service`/`Resource`/`ProviderAvailability` types — *verified in repo: `Service`, `Resource`, `ProviderAvailability` are complete and correct.* `Business` is missing the same two fields noted above.
...
**Exit criteria:** owner signs up, logs in, creates a business, adds a service and a staff member, sees it in the dashboard — end to end. **Not yet met** — blocked today by: no `/login` route, `authenticate` unwired into any protected route, and no `StaffInvitations` flow to add a staff member with.
...
| Phase 2 — Foundations (auth/tenant/staff/service) | 🟡 In progress — signup, sessions, Service/Resource/ProviderAvailability CRUD, and provider validation all real and correct; blocked on `authenticate` being unwired, no `/login`, no `StaffInvitations` (see Phase 2 audit note, 2026-09-03) | — |
